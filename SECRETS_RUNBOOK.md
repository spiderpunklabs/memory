...
> **Default is rotate, not rewrite.** History rewrites break clones and can cause more damage than the leak itself. Only rewrite for very high-value credentials that cannot be rotated.

If rewrite is required, see the full playbook in
[`spiderpunklabs/workstation/ci-templates/SECRETS_RUNBOOK.md`](https://github.com/spiderpunklabs/workstation/blob/main/ci-templates/SECRETS_RUNBOOK.md).

## Document

After resolution, add an entry to `.gitleaks.toml` allowlist if the finding will recur as a known false positive. Each allowlist entry MUST include a `description` explaining why the finding is benign.
